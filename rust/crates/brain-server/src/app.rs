...
fn build_router(state: Arc<AppState>, static_dir: Option<&Path>) -> Router {
    let mut app = Router::new()
        // HTML pages (7 main + old URLs kept for bookmarks)
        .route("/", get(routes::dashboard))
        .route("/goals", get(routes::goals))
        .route("/cognition", get(routes::cognition))
        .route("/imagine", get(routes::imagine))
        ...
        .route("/interact", get(routes::explore))
        .route("/brain", get(routes::explore))
        .route("/youtube", get(routes::cognition))
        .route("/chat", get(routes::face))
        .route("/listen", get(routes::face))
        .route("/watch", get(routes::face))
        // YouTube Brain proxy
